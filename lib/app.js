...
class App {
    ...
    _saveSelectionAsText(textKey, editor) {
        ...
        return this._textRegistry.set(textKey, this._editorTextExtractor.extract(editor), fileName, selection);
    }

    _getTitlePart(item) {
        return `${item.fileName}${item.selection ? `:${item.selection.start.line}-${item.selection.end.line}` : ''}`;
    }
