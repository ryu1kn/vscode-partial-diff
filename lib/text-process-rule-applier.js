...
class TextProcessRuleApplier {
    ...
    _applyRuleToText(rule, text) {
        ...
        if (typeof rule.replaceWith === 'string') return text.replace(regex, rule.replaceWith);

        return text.replace(regex, matched => {
            switch (rule.replaceWith.letterCase) {
                case 'lower': return matched.toLowerCase();
                case 'upper': return matched.toUpperCase();
