...
class Bootstrapper {
    ...
    _registerProviders(context) {
        const disposable = this._vscode.workspace.registerTextDocumentContentProvider(
            EXTENSION_SCHEME, this._contentProvider
        );
    ...
    _registerCommands(context) {
        ...
        const commandMap = new Map([
            ...
            [`${EXTENSION_NAMESPACE}.markSection2AndTakeDiff`, factory.createCompareSelectionWithText1Command()],
            [`${EXTENSION_NAMESPACE}.diffSelectionWithClipboard`, factory.createCompareSelectionWithClipboardCommand()]
        ]);
        commandMap.forEach((command, commandName) => {
            const handler = command.execute.bind(command);
            const disposable = this._vscode.commands.registerTextEditorCommand(commandName, handler);
            context.subscriptions.push(disposable);
        });
    }
