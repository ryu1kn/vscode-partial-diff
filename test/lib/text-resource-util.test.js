...
suite('TextResourceUtil', () => {
    ...
    suite('#getUri', () => {
        ...
        test('it converts a given text key into an uri', () => {
            ...
            const textResourceUtil = new TextResourceUtil({extensionScheme, Uri, getCurrentDateFn});
            expect(textResourceUtil.getUri('1')).to.eql('__EXTENSION_SCHEME:text/1?_ts=1465990980000__');
        });
    });
    ...
    suite('#getTextKey', () => {
        ...
        test('it extracts a text key information from the given uri', () => {
            const uri = {path: 'text/1'};
            const textResourceUtil = new TextResourceUtil({});
            expect(textResourceUtil.getTextKey(uri)).to.eql('1');
        });
    });
});
